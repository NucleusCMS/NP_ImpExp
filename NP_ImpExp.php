	...
	function _convertMedia($matches) {
		...
		switch($type){
			...
			case 'popup':
				...
				$attr .= $w ? " width=\"$w\"": '' ;
				$attr .= $h ? " hight=\"$h\"": '' ;
				$attr .= $alt ? " alt=\"$alt\" title=\"$alt\"": " alt=\"$filename\" title=\"$filename\"" ;

				$title = ( $alt ) ? $alt : $filename;
				$mediatag = "<img src=\"{$CONF['MediaURL']}{$collection}/{$filename}\"$attr />";
				break;
				
			case 'media':
				$title = ( $alt ) ? $alt : $filename;
				$mediatag = "<a href=\"{$CONF['MediaURL']}{$collection}/{$filename}\">$title</a>";
				break;
		}
